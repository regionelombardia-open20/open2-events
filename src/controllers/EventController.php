...
class EventController extends base\EventController
{
        ...
        if ($register) {
            ...
            try {
                ...
                if ($invitation->type == EventInvitation::INVITATION_TYPE_IMPORTED) {
                    ...
                    if (isset($userNew['user']) && !is_null($userNew['user'])) {
                        ...
                        if (isset($userNew['error']) && ($userNew['error'] == UserProfileUtility::UNABLE_TO_CREATE_USER_ERROR)) {
                            $ses->addFlash('info', AmosEvents::t('amosevents', '{username} Procedere con il login', ['username' => $userNew['messages']['email'][0]]));
                            ...
                            $ses->addFlash('danger', AmosEvents::t('amosevents', 'Qualcosa è andato storto nella creazione dell\'utente'));
                            return $this->redirect('/');
                        }
                    }
                } else {
                ...
                if (!is_null($communityModule)) {
                    $communityModule->createCommunityUser($event->community_id,
                        $community_memeber_status,
                        Event::EVENT_PARTICIPANT,
                        $userId,
                        $invitation->invitation_sent_on,
                        new \yii\db\Expression('now()'),
                        $invitation->partner_of);
                }
                ...
                if ($partners) {
                    ...
                    foreach ($partners as $partner) {
                        if ($partner['email']) {
                            ...
                            // Sends e-mail
                            $urlYes = Url::base(true) . Url::toRoute(['subscribe-and-register', 'id' => $eventId, 'code' => $partnerInvitation->code]);
                            $urlNo = Url::base(true) . Url::toRoute(['reject', 'id' => $eventId, 'code' => $partnerInvitation->code]);
                            ...
                            Email::sendMail($from, [$partner['email']], 'Invito - ' . $event->title, $text, [], [], [], 0, false);
                        }
                    }
                }
    ...
    public function actionSubscribeUserToEvent()
    {
        ...
        $defaultAction = ['/events/event/associate-user-to-event-m2m', 'id' => $userProfile->id, 'viewM2MWidgetGenericSearch' => true];
        ...
        // Email subject
        $subject = AmosEvents::t('amosevents', '#user_invite_user_to_event_email_subject', [
        ...
        $text .= AmosEvents::t('amosevents', 'Title') . ': ' . $event->title . '<br>';
        $text .= AmosEvents::t('amosevents', 'Summary') . ': ' . $event->summary . '<br>';
        $text .= $event->getAttributeLabel('begin_date_hour') . ': ' . Yii::$app->getFormatter()->asDatetime($event->begin_date_hour, 'humanalwaysdatetime') . '<br>';
        ...
        if (
            ...
            $text .=
                AmosEvents::t('amosevents', 'Location') . ': ' .
        ...
        if ($ok) {
            ...
            Yii::$app->getSession()->addFlash('danger', AmosEvents::t('amosevents', 'An error occurred while sending notification email'));
        }
     ...
     * @see \open20\amos\events\models\EventInvitationsUpload::parse()
     */
        ...
        foreach ($rows as $r => $row) {
            try {
                ...
                if ($this->eventsModule->enableAutoInviteUsers && ($row['type'] == EventInvitation::INVITATION_TYPE_REGISTERED)) {
                    ...
                    $extUrlYes = Url::base(true) . Url::toRoute(['event-signup', 'eid' => $event->id, 'pName' => $row['name'], 'pSurname' => $row['surname'], 'pEmail' => $row['email'], 'pCode' => $row['code']]);
                    $regUrlNo = Url::base(true) . Url::toRoute(['reject', 'id' => $event->id]);
                    $row['email'] = $user['email'];
                    $text = $this->renderPartial('email_invitation_registered', [
                        'event' => $event,
                        'user' => $user,
                        'profile' => $profile,
                        'urlYes' => $extUrlYes,
                    ...
                    $extUrlYes = Url::base(true) . Url::toRoute(['event-signup', 'eid' => $event->id, 'pName' => $row['name'], 'pSurname' => $row['surname'], 'pEmail' => $row['email']]);
                    $text = $this->renderPartial('email_invitation', [
                        'event' => $event,
                        'user' => $row,
                        'urlYes' => $extUrlYes
                    ]);
                ...
                // Sends e-mail
                $ok = Email::sendMail($from, [$row['email']], 'Invito - ' . $event->title, $text, [], [], [], 0, false);
    ...
    public function subscribeToEventCommunity($eid, $user, $communityMemeberStatus = '')
    {
        ...
        if ($communityUserMm->save()) {
            Yii::$app->getSession()->addFlash('success', AmosEvents::t('amosevents', 'You have been registered to the event successfully.'));
        } else {
            Yii::$app->getSession()->addFlash('danger', AmosEvents::t('amosevents', 'There were some problems with your registration to the event'));
        }
                        ...
                        if($eventParticipantModel->validate() && $modelValidations){
                            if ((count($invitationFound) == 0) || ($this->eventsModule->enableAutoInviteUsers && (count($invitationFound) > 0))) {
                                ...
                                if($gdprMarked) {
                                    ...
                                    if($event->event_type_id == EventType::TYPE_LIMITED_SEATS &&
                                        ...
                                        if(!$user) {
                                            ...
                                            $newUser = AmosAdmin::getInstance()->createNewAccount(
                                                $participantData['nome'], $participantData['cognome'], $participantData['email'], 0
                                            );
                                            $user = $newUser['user'];
                                        ...
                                        $subscribedToEventCommunity = $this->subscribeToEventCommunity($eid, $user, $communityMemeberStatus);
                                    ...
                                    \Yii::$app->getSession()->addFlash('danger', AmosEvents::txt('Compilare tutte le domande relative alle condizioni e l\'uso dei dati personali'));
                                    return $this->render((!empty($event->subscribe_form_page_view) ? $event->subscribe_form_page_view : 'event_signup'), [
                                        'event' => $event,
                                        'userData' => $participantData,
                                        'eventParticipantModel' => $eventParticipantModel,
                                ...
                                \Yii::$app->getSession()->addFlash('danger', AmosEvents::txt('This user has already been registered at this event'));
                            }
                        } else {
                            \Yii::$app->getSession()->addFlash('danger', AmosEvents::txt('Controllare riempimento dei campi obbligatori'));
                        }

                    } else {
                        if(\Yii::$app->user) {
                            $user = User::find()->andWhere(['id' => \Yii::$app->user->id])->andWhere(['deleted_at' => null, 'deleted_by' => null])->asArray()->one();
                            /** @var UserProfile $userProfile */
                            $userProfile = UserProfile::find()->andWhere(['user_id' => \Yii::$app->user->id])->andWhere(['deleted_at' => null, 'deleted_by' => null])->asArray()->one();
                        }
        ...
        if ($event) {
            ...
            if($event['has_tickets'] &&
                ...
                if($user) {
                    ...
                    /** @var UserProfile $userProfile */
                    $userProfile = UserProfile::find()->andWhere(['user_id' => $user['id']])->andWhere(['deleted_at' => null, 'deleted_by' => null])->asArray()->one();
                            ...
                            if(!$invitation->presenza) {
                                ...
                                if($event['seats_management']){
                                    ...
                                    if($seat && $seat->status == EventSeats::STATUS_REASSIGNED){
                                        \Yii::$app->getSession()->addFlash('warning', AmosEvents::txt('Attenzione! Il  posto <strong>{seat}</strong> è stato riassegnato. Reindirizzare al desk accrediti!', [
                                            'nomeCognome' => $userProfile['nome'] . ' ' . $userProfile['cognome'],
                                            'seat' => $seat->getStringCoordinateSeat()
                                        ]));
                                    }
                                }
                                \Yii::$app->getSession()->addFlash('success', AmosEvents::txt('#user_registered', ['name_surname' => $userProfile['nome'] . ' ' . $userProfile['cognome']]));
                            } else {
                                \Yii::$app->getSession()->addFlash('danger', AmosEvents::txt('Already marked as attendant', ['name_surname' => $userProfile['nome'] . ' ' . $userProfile['cognome']]));
                            }
        ...
        if ($event) {
            if($event['has_tickets'] &&
                ...
                if ($companion) {
                    ...
                    if ($user) {
                        $invitation = $eventInvitationModel::find()->andWhere(['id' => $iid])->andWhere(['deleted_at' => null, 'deleted_by' => null])->asArray()->one();
                        if ($invitation) {
                            if ($invitation['user_id'] == $pid && $invitation['event_id'] == $eid && $companion->event_invitation_id == $iid) {
                                if (!$companion->presenza) {
                                    ...
                                    \Yii::$app->getSession()->addFlash('success', AmosEvents::txt('#companion_registered', ['name_surname' => $companion->nome . ' ' . $companion->cognome]));
                                } else {
                                    \Yii::$app->getSession()->addFlash('danger', AmosEvents::txt('Already marked as attendant', ['name_surname' => $companion['nome'] . ' ' . $companion['cognome']]));
                                }
                            } else {
                                if ($booleanResponse) {
                                    return false;
                ...
                if ($invitation) {
                    ...
                    $content = $this->renderPartial(
                        ...
                        [
                            ...
                            'seatModel' => $seatModel,
                            'qrcode' => $event->has_qr_code
                                ? EventsUtility::createQrCode($event, $invitation, 'participant', null, null, 'png')
                                : '',
                        ]
                    );
                    ...
                    foreach ($companions as $companion) {
                        ...
                        $content .= $this->renderPartial(!empty($event->ticket_layout_view) ? $event->ticket_layout_view : 'pdf-tickets/general-layout', [
                            ...
                            'seatModel' => $seatModel,
                            'qrcode' => $event->has_qr_code ? EventsUtility::createQrCode($event, $invitation, 'companion', $companion, null, 'png') : "",
                        ]);
                    }
    ...
    public function actionShowCompanionsListOnly($eid = null, $iid = null)
    {
        ...
        if(empty($iid)) {
            ...
            if($expandRowKey && !empty($expandRowKey)) {
                $invitation = $eventInvitationModel::find()->andWhere(['user_id' => CommunityUserMm::findOne(['id' => $expandRowKey])->user_id, 'event_id' => $eid])->one();
     ...
     */
    public function sendSignupConfirmEmail($eid, $iid) {
        ...
        $companions = $eventParticipantCompanionModel::find()->andWhere(['event_invitation_id' => $invitation->id])->andWhere(['deleted_at' => null, 'deleted_by' => null])->asArray()->all();
        ...
        $userProfile = $user->getProfile();
        // Populate SUBJECT
        $subject = AmosEvents::t('amosevents', 'Signup confirm for event') . ' ' . $event->title;
    ...
    public function sendTicket($eid, $iid) {
        ...
        if($invitation) {
            ...
            if (!empty(trim($event->email_ticket_subject))) {
            ...
            if (!empty($participants)) {
                ...
                foreach ($participants as $participant) {
                    $results[] = [
                        ...
                        'accreditation_list' => !empty($participant->getAccreditationList()->one())
                                                    ? $participant->getAccreditationList()->one()->title
                                                    : '',
                        'ticket_sent' => ($participant->is_ticket_sent
                                            ? Yii::t('amoscore', 'Yes')
                                            : Yii::t('amoscore', 'No')
                                         ),
                        'downloadedat' => !empty($participant->ticket_downloaded_at)
                                            ? date("d-m-Y H:i:s", strtotime($participant->ticket_downloaded_at))
                                            : '',
                        'downloadedby' => !empty($participant->ticket_downloaded_by)
                                            ? (!empty(UserProfile::findOne(['user_id' => $participant->ticket_downloaded_by]))
                                                ? (UserProfile::findOne(['user_id' => $participant->ticket_downloaded_by])['nomeCognome'])
                                                : '')
                                            : '',
                        'attendant' => ($participant->presenza
                                        ? Yii::t('amoscore', 'Yes')
                                        : Yii::t('amoscore', 'No')
                                       ),
                    ];
                    ...
                    if(!empty($companions)) {
                        ...
                        foreach ($companions as $companion) {
                            $results[] = [
                                ...
                                'accreditation_list' => !empty($companion->getAccreditationList()->one())
                                                            ? $companion->getAccreditationList()->one()->title
                                                            : '',
                                'ticket_sent' => '',
                                'downloadedat' => '',
                                'downloadedby' => '',
                                'attendant' => ($companion->presenza
                                                    ? Yii::t('amoscore', 'Yes')
                                                    : Yii::t('amoscore', 'No')
                                               ),
                            ];
...
//                                    ($participantsWantToJoin > ($event->seats_available - $event->checkParticipantsQuantity())) ||
                                ...
                                if ($user && $user->userProfile->attivo) {
                                    ...
                                    $participant = $this->addParticipant($eid, $dataParticipant, $user->id, $gdprQuestions, $invitation, true);
                                    ...
                                    for ($i = 1; $i <= $registerGroupForm->nSeats - 1; $i++) {
                                        ...
                                        $companions [] = $this->addCompanion($eid, $participant, $dataCompanion);
                                    }

                                    if ($event->seats_management) {
                                        $this->assignSeats($this->model, $participant, $companions, $registerGroupForm->sector);
                                    }

                                    $this->sendSignupConfirmEmail($event->id, $participant->id);
                                    \Yii::$app->session->addFlash('success', AmosEvents::t('amosevents', "E' stato iscritto un gruppo di {n} persone.", ['n' => $registerGroupForm->nSeats]));
...
//                            ]);
                                    ...
                                    \Yii::$app->getSession()->addFlash('danger', AmosEvents::txt('La email del gruppo inserita è già presente nella piattaforma ma l\'utente non risulta più attivo, occorre cambiare la email o riattivare l\'utente'));
                                }
                            }
                        } else {
                            \Yii::$app->getSession()->addFlash('danger', AmosEvents::txt('Compilare tutte le domande relative alle condizioni e l\'uso dei dati personali'));
    ...
    public function actionAssignSeat($id, $user_id = null, $event_companion_id = null){
        ...
        if(\Yii::$app->request->post() && $formModel->load(\Yii::$app->request->post())){
            ...
            if($seat) {
                ...
                if ($seat->save()) {
                    \Yii::$app->session->addFlash('success', AmosEvents::t('amosevents', "Posto settore {sector} - fila {row} - posto {seat} assegnato correttamente all'utente {nomeCognome}", [
                        'row' => $formModel->row,
                    ...
                    \Yii::$app->session->addFlash('danger', AmosEvents::t('amosevents', "Errore nell'assegnamento del posto"));
                }
            }
        }
